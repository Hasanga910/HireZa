...
<%
    ...
    String currentRole = "JobSeeker"; // This page is specifically for JobSeekers

    // ✅ Fetch from session (because we are using sendRedirect)
    List<User> jobSeekers = (List<User>) session.getAttribute("users");
    String selectedRole = (String) session.getAttribute("selectedRole");

    // ✅ Debug logging (remove in production)
    System.out.println("JSP - Selected Role: " + selectedRole);
    System.out.println("JSP - Users count: " + (jobSeekers != null ? jobSeekers.size() : 0));

    // ✅ If no data OR wrong role data, redirect to fetch correct data
    if (jobSeekers == null || !currentRole.equals(selectedRole)) {
        System.out.println("JSP - Redirecting to fetch JobSeeker data");
        response.sendRedirect(request.getContextPath() + "/UserManagementServlet?action=fetch&userRole=JobSeeker");
        return;
    }
    ...
    if (jobSeekers == null) jobSeekers = new java.util.ArrayList<>();
...
<head>
    ...
    <style>
        /* Stats Cards Styles - Blue Theme for Job Seekers */
        .stats-cards {
            display: grid;
        ...
        /* Search and Filters Styles - Blue Theme */
        .search-filters {
            background: #ffffff;
...
<div class="sidebar" id="sidebar">
    ...
    <ul class="sidebar-nav">
        ...
        <li class="nav-item">
            <a href="${pageContext.request.contextPath}/UserManagementServlet?action=fetch&userRole=JobSeeker" class="nav-link active">
...
<div class="main-content" id="mainContent">
    ...
    <div class="top-navbar">
        <div class="navbar-left">
            ...
            <h1 class="page-title" id="pageTitle">Job Seekers</h1>
        </div>
    ...
    <div class="dashboard-content">
        <div class="welcome-section">
            <h2><i class="fas fa-users me-2"></i>Job Seekers Management</h2>
            <p class="mb-0">Manage job seeker accounts and monitor their application activities on the platform.</p>
        </div>
        ...
        <div class="stats-cards" id="statsCards">
            <div class="stat-card">
                ...
                <h3 id="totalJobSeekers"><%= totalCount %></h3>
        ...
        <div class="search-filters">
            <div class="row">
                <div class="col-md-8">
                    <label for="searchJobSeeker" class="form-label">Search Job Seekers</label>
                    <input type="text" class="form-control" id="searchJobSeeker" placeholder="Search by name, email, username, phone...">
                </div>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">
                    Job Seekers List <span class="badge bg-primary" id="filteredCount"><%= totalCount %></span>
                </h5>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover" id="jobSeekerTable">
                        ...
                        <tbody>
                        <% for (User js : jobSeekers) { %>
                        <tr data-user-id="<%= js.getId() %>">
                            <td>
                                <span class="badge bg-primary"><%= js.getId() %></span>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="user-avatar me-2">
                                        <%= js.getFullName() != null && !js.getFullName().isEmpty() ?
                                                js.getFullName().charAt(0) : "?" %>
                                    </div>
                                    <%= js.getFullName() != null ? js.getFullName() : "N/A" %>
                                </div>
                            </td>
                            <td><%= js.getUsername() != null ? js.getUsername() : "N/A" %></td>
                            <td>
                                <% if (js.getEmail() != null && !js.getEmail().isEmpty()) { %>
                                <a href="mailto:<%= js.getEmail() %>" class="text-decoration-none">
                                    <%= js.getEmail() %>
                                </a>
                                <% } else { %>
                                N/A
                            ...
                            <td>
                                <% if (js.getPhone() != null && !js.getPhone().isEmpty()) { %>
                                <a href="tel:<%= js.getPhone() %>" class="text-decoration-none">
                                    <%= js.getPhone() %>
                                </a>
                                <% } else { %>
                                N/A
                            ...
                            <td>
                                <button type="button" class="btn btn-sm btn-outline-danger"
                                        onclick="deleteUser('<%= js.getId() %>', '<%= js.getFullName() != null ? js.getFullName().replace("'", "\\'") : "this job seeker" %>')">
                ...
                <div class="text-center text-muted py-5">
                    ...
                    <h5>No job seekers found</h5>
                    <p>Make sure there are users with role "JobSeeker" in your database.</p>
                </div>
                <% } %>
...
<script>
    ...
    async function fetchJobApplicationStats() {
        try {
            const response = await fetch('${pageContext.request.contextPath}/JobSeekerStatsServlet');
            const data = await response.json();

            if (data.success) {
                animateNumber('applications', data.totalApplications || 0);
                animateNumber('profilecompleted', data.profilesCompleted || 0);
            } else {
                console.error('Error fetching stats:', data.error);
                // Set default values if fetch fails
                animateNumber('applications', 0);
                animateNumber('profilecompleted', 0);
            }
        } catch (error) {
    ...
    function storeOriginalTableRows() {
        ...
        if (tableBody) {
            ...
            filteredJobSeekers = [...originalTableRows];
        }
    }

    function initializeSearch() {
        const searchInput = document.getElementById('searchJobSeeker');
    ...
    function calculateStatistics() {
        const totalJobSeekers = originalTableRows.length;

        // Update stats with animation
        animateNumber('totalJobSeekers', totalJobSeekers);
    }
    ...
    function applyFilters() {
        const searchTerm = document.getElementById('searchJobSeeker').value.toLowerCase(); // Change ID based on page

        filteredJobSeekers = originalTableRows.filter(row => { // Change variable name based on page
            ...
            // Search filter - check all relevant fields including User ID
    ...
    function updateTableDisplay() {
        const tableBody = document.querySelector('#jobSeekerTable tbody'); // Change ID based on page
        ...
        if (filteredJobSeekers.length === 0) { // Change variable name based on page
            ...
            filteredJobSeekers.forEach((row) => { // Change variable name based on page
                const newRow = row.cloneNode(true);
                // Keep the original User ID (no sequential numbering)
    ...
    function updateFilteredCount() {
        ...
        if (countElement) {
            countElement.textContent = filteredJobSeekers.length;
        }
    }

    function clearFilters() {
        // Clear search input
        document.getElementById('searchJobSeeker').value = '';
    ...
    function setActiveNavigation() {
        ...
        // Set active for job seekers page specifically
        const jobSeekerLinks = document.querySelectorAll('a[href*="userRole=JobSeeker"]');
    ...
    function updatePageTitle() {
        ...
        if (pageTitle) {
            pageTitle.textContent = 'Job Seekers';
        }
    }
    ...
    function refreshData() {
        window.location.href = '${pageContext.request.contextPath}/UserManagementServlet?action=fetch&userRole=JobSeeker';
    }

    // ✅ Delete user function
    function deleteUser(userId, fullName) {
        if (confirm('Are you sure you want to delete user: ' + fullName + '?')) {
            window.location.href = '${pageContext.request.contextPath}/UserManagementServlet?action=delete&userId=' + userId + '&userRole=JobSeeker';
        }
    }
