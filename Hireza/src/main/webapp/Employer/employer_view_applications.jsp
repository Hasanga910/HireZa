...
<%@ page import="model.User, dao.CompanyDAO, model.Company, model.JobApplication, java.util.List, dao.RecruiterDAO, model.Recruiter, dao.UserDAO, model.User, dao.JobPostDAO, model.JobPost" %>
<%
...
<head>
    ...
    <title>Employer View Applications - Job Portal</title>
...
<%--            <li><a href="${pageContext.request.contextPath}/company/employer/applications"><i class="fas fa-file-alt"></i> <span>Employer View Applications</span></a></li>--%>
            ...
            <li><a href="${pageContext.request.contextPath}/company/employer/applications" class="active"><i class="fas fa-tachometer-alt"></i> <span>Employer View Applications</span></a></li>
    ...
    <main class="main-content" id="mainContent">
        <div class="header">
            <h1>Applications for Job ID: <%= request.getAttribute("jobId") %></h1>
        </div>
        <%
        ...
        <div class="job-details">
            <h3>Job: <%= job.getJobTitle() %> (<%= job.getLocation() != null ? job.getLocation() : "N/A" %> - <%= job.getWorkMode() != null ? job.getWorkMode() : "N/A" %>)</h3>
            <p><strong>Applications Received:</strong> <%= applications != null ? applications.size() : 0 %></p>
        </div>
        <% } %>
        ...
        <table id="applicationsTable">
            ...
            <%
                ...
                for (JobApplication app : applications) {
                    ...
                    String recruiterId = app.getRecruiterId();
                    if (recruiterId != null && !recruiterId.isEmpty()) {
                        Recruiter recruiter = recruiterDAO.getRecruiterById(recruiterId);
                        if (recruiter != null) {
                            User recruiterUser = userDAO.getUserById(recruiter.getUserId());
                            if (recruiterUser != null) {
                                updatedBy = recruiterUser.getFullName() != null ? recruiterUser.getFullName() : recruiterUser.getUsername();
                            }
                        }
                    }
            %>
            <tr>
                ...
                <td><%= app.getFullName() != null ? app.getFullName() : "N/A" %></td>
                <td><%= app.getEmail() != null ? app.getEmail() : "N/A" %></td>
                <td><%= app.getPhone() != null ? app.getPhone() : "N/A" %></td>
                <td><%= app.getStatus() != null ? app.getStatus() : "N/A" %></td>
                <td><%= updatedBy %></td>
                <td><%= app.getAppliedAt() != null ? app.getAppliedAt() : "N/A" %></td>
                <td>
                    <a href="${pageContext.request.contextPath}/Employer/view_applicant_cv.jsp?applicationId=<%= app.getApplicationId() %>&jobId=<%= jobId %>" class="btn" onclick="showNotification('Viewing CV for application ID: <%= app.getApplicationId() %>')"><i class="fas fa-file-alt"></i> View CV</a>
                </td>
            </tr>
            <% } %>
            </tbody>
        ...
        <div class="pagination">
            ...
            <% for (int i = 1; i <= Math.ceil(applications.size() / 10.0); i++) { %>
            <a href="#" onclick="showPage(<%= i %>); showNotification('Navigating to page <%= i %>')"><%= i %></a>
            <% } %>
            <% } %>
        </div>
        <% } %>
        <a href="${pageContext.request.contextPath}/company/employer/applications" class="btn btn-secondary" onclick="showNotification('Returning to Job Selection')"><i class="fas fa-arrow-left"></i> Back to Job Selection</a>
    </main>
</div>
...
<script>
    ...
    function filterTable() {
        ...
        for (let i = 1; i < tr.length; i++) {
            ...
            if (tdName && tdEmail) {
                tr[i].style.display = (tdName.textContent.toLowerCase().indexOf(filter) > -1 || tdEmail.textContent.toLowerCase().indexOf(filter) > -1) ? '' : 'none';
            }
        }
        if (filter) {
            showNotification('Searching for: ' + filter);
        }
    }
    ...
    function showPage(pageNum) {
        ...
        for (let i = 1; i < rows.length; i++) {
            rows[i].style.display = (i >= (pageNum - 1) * rowsPerPage + 1 && i < pageNum * rowsPerPage + 1) ? '' : 'none';
        }
    }
