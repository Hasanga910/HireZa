...
public class CompanyProfileServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/Employer/company_profile.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Company company = new Company(user.getId(), companyName, industry, description, website, address, city, state,
                zipCode, contactNumber, companySize, foundedYear, workMode, employmentTypes, companyEmail, logoBytes, aboutUs);

        // Check if profile exists to decide between save or update
        boolean isSaved;
        if (companyDAO.getCompanyByUserId(user.getId()) == null) {
            isSaved = companyDAO.saveCompanyProfile(company);
        } else {
            isSaved = companyDAO.updateCompanyProfile(company);
        }

        if (isSaved) {
            response.sendRedirect(request.getContextPath() + "/Employer/employer_dashboard.jsp");
        } else {
            response.sendRedirect(request.getContextPath() + "/Employer/company_profile.jsp?error=1");
        }
    }
}